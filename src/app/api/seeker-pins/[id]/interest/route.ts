...
// POST /api/seeker-pins/[id]/interest — instant, on-demand contact request
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: seekerPin, error: seekerPinError } = await supabase
    .from('seeker_pins')
    .select('id, status')
    .eq('id', id)
    .maybeSingle();
  ...
  if (seekerPinError) {
    console.error('Failed to load seeker_pin for interest request:', JSON.stringify(seekerPinError));
    return NextResponse.json({ error: 'Failed to record interest' }, { status: 500 });
  }
  if (!seekerPin) {
    return NextResponse.json({ error: 'Seeker pin not found' }, { status: 404 });
  }
  if (seekerPin.status !== 'active') {
    return NextResponse.json({ error: 'This search is no longer active' }, { status: 400 });
  }

  const { error: insertError } = await supabase.from('interest_requests').insert({
    target_type: 'seeker_pin',
    target_seeker_pin_id: id,
